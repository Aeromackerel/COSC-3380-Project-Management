...
$parse = $_GET['report'];
$keywords = preg_split("/[\s,]+/",$parse);
$employeeId = $keywords[1];
$projectId = $keywords[0];
...
//total Hours
$sqlTotalHoursQuery = "SELECT SUM(hours) as allHours FROM Timesheet WHERE employeeId = $employeeId AND projectId = $projectId";
$stmt6 = $conn->query($sqlTotalHoursQuery);
$row6 = $stmt6->fetch(PDO::FETCH_ASSOC);
$totalHours = $row6['allHours'];

//total tasks
$sqlTotalProjectTasksQuery = "SELECT COUNT(taskId) as totalTasks FROM Tasks WHERE employeeId = $employeeId AND projectId = $projectId";
...
//tasks completed
$completeStatus=5;
$sqlProjectTasksCompletedQuery = "SELECT COUNT(taskId) as completedTasks FROM Tasks WHERE employeeId = $employeeId AND status = $completeStatus AND projectId = $projectId";
$stmt3 = $conn->query($sqlProjectTasksCompletedQuery);
$row3 = $stmt3->fetch(PDO::FETCH_ASSOC);
$tasksCompleted = $row3['completedTasks'];

//tasks in progress
$sqlProjectTasksInProgressQuery = "SELECT COUNT(taskId) as progressTasks FROM Tasks WHERE employeeId = $employeeId AND (status = 2 OR status = 3 OR status = 4) AND projectId = $projectId";
...
//tasks not started
$notStartedStatus=1;
$sqlProjectTasksCompletedQuery = "SELECT COUNT(taskId) as notStartedTasks FROM Tasks WHERE employeeId = $employeeId AND status = $notStartedStatus AND projectId = $projectId";
...
<body>
	...
	<div id = "login-container" class = "ui-container">
		...
		// Include DB Connection
		//include "../../../includes/dbconnect.ini.php";
		$sqlProjectNameQuery = "Select firstName, lastName FROM EMPLOYEES WHERE employeeId = $employeeId";
		$stmt = $conn->query($sqlProjectNameQuery);
		$row = $stmt->fetch(PDO::FETCH_ASSOC);
		echo "<center><p> Project Hours Report on $row[firstName] $row[lastName] from $from to $to </p></center>"
		?>
