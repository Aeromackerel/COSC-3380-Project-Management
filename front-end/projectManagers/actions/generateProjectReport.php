...
//total Hours
$sqlTotalHoursQuery = "SELECT SUM(hours) as allHours FROM Timesheet WHERE projectId = $projectId AND DATEDIFF(day, date, '$from') <= 0 AND DATEDIFF(day, date, '$to') >=0";
$stmt6 = $conn->query($sqlTotalHoursQuery);
$row6 = $stmt6->fetch(PDO::FETCH_ASSOC);
$totalHours = $row6['allHours'];
//total tasks
$sqlTotalProjectTasksQuery = "SELECT COUNT(taskId) as totalTasks FROM Tasks WHERE projectId = $projectId AND DATEDIFF(day, startDate, '$from') <= 0 AND DATEDIFF(day, startDate, '$to') >=0";
$stmt2 = $conn->query($sqlTotalProjectTasksQuery);
$row2 = $stmt2->fetch(PDO::FETCH_ASSOC);
$totalTasks = $row2['totalTasks'];
//tasks completed
$completeStatus=5;
$sqlProjectTasksCompletedQuery = "SELECT COUNT(taskId) as completedTasks FROM Tasks WHERE projectId = $projectId AND status = $completeStatus AND DATEDIFF(day, startDate, '$from') <= 0 AND DATEDIFF(day, startDate, '$to') >=0";
$stmt3 = $conn->query($sqlProjectTasksCompletedQuery);
$row3 = $stmt3->fetch(PDO::FETCH_ASSOC);
$tasksCompleted = $row3['completedTasks'];
//tasks in progress
$sqlProjectTasksInProgressQuery = "SELECT COUNT(taskId) as progressTasks FROM Tasks WHERE projectId = $projectId AND (status = 2 OR status = 3 OR status = 4) AND DATEDIFF(day, startDate, '$from') <= 0 AND DATEDIFF(day, startDate, '$to') >=0";
$stmt4 = $conn->query($sqlProjectTasksInProgressQuery);
$row4 = $stmt4->fetch(PDO::FETCH_ASSOC);
$tasksInProgress = $row4['progressTasks'];
//tasks not started
$notStartedStatus=1;
$sqlProjectTasksCompletedQuery = "SELECT COUNT(taskId) as notStartedTasks FROM Tasks WHERE projectId = $projectId AND status = $notStartedStatus AND DATEDIFF(day, startDate, '$from') <= 0 AND DATEDIFF(day, startDate, '$to') >=0";
