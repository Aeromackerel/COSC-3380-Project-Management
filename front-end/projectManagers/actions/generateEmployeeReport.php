...
//total Hours
$sqlTotalHoursQuery = "SELECT SUM(hours) as allHours FROM Timesheet WHERE employeeId = $employeeId";
$stmt6 = $conn->query($sqlTotalHoursQuery);
$row6 = $stmt6->fetch(PDO::FETCH_ASSOC);
$totalHours = $row6['allHours'];

//total tasks
$sqlTotalProjectTasksQuery = "SELECT COUNT(taskId) as totalTasks FROM Tasks WHERE employeeId = $employeeId";
...
//tasks completed
$completeStatus=5;
$sqlProjectTasksCompletedQuery = "SELECT COUNT(taskId) as completedTasks FROM Tasks WHERE employeeId = $employeeId AND status = $completeStatus";
$stmt3 = $conn->query($sqlProjectTasksCompletedQuery);
$row3 = $stmt3->fetch(PDO::FETCH_ASSOC);
$tasksCompleted = $row3['completedTasks'];

//tasks in progress
$sqlProjectTasksInProgressQuery = "SELECT COUNT(taskId) as progressTasks FROM Tasks WHERE employeeId = $employeeId AND (status = 2 OR status = 3 OR status = 4)";
...
//tasks not started
$notStartedStatus=1;
$sqlProjectTasksCompletedQuery = "SELECT COUNT(taskId) as notStartedTasks FROM Tasks WHERE employeeId = $employeeId AND status = $notStartedStatus";
